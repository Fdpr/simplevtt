...
var defaultPath = app.getAppPath() + "\\res"

// load language locale
function loadLanguage(){
    let data;
    try{
        data = fs.readFileSync(app.getAppPath() + "/config/lang.json");
...
function loadConfig(){
    let data = fs.readFileSync(app.getAppPath() + "/config/config.json");
    if (data)
        return JSON.parse(data);
    else
        return null;
...
function loadImageFile(){
    ...
    if (files != null && files.length > 0){
        let paths = files[0].split("\\")
        paths.pop();
        defaultPath = paths.join("\\");
        return files[0]
    } else
        return null;
}
...
function loadVTT(path){
    ...
    for (let i = 0; i < vtt.backgrounds.length; i++){
        ...
        image.onload = function(){
            ...
            bgg.killed = vtt.backgrounds[i].killed;
    ...
    for (let i = 0; i < vtt.tokens.length; i++){
        ...
        image.onload = function(){
            ...
            tkk.killed = vtt.tokens[i].killed;
